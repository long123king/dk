...
void RegisterCmdHandlers()
{
    ...
    CMD_LIST->RegisterCmdHandler("serve_start",
        CMD_HANDLER(serve_start),
        "Start embedded HTTP server (phase 1)",
        "!dk serve_start [host] [port]");

    // CMD_LIST->RegisterCmdHandler("serve_stop",
    //     CMD_HANDLER(serve_stop),
    //     "Stop embedded HTTP server",
    //     "!dk serve_stop");

    // CMD_LIST->RegisterCmdHandler("serve_status",
    //     CMD_HANDLER(serve_status),
    //     "Show embedded HTTP server status",
    //     "!dk serve_status");
...
void CCmdList::PrintAllHelp()
{
    ...
    const size_t kDescWidth = 80; // ���������ʾ��ô���ַ��������ض�
